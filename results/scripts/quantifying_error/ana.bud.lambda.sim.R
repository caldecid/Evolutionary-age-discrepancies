###############################################################################
###########    Simulation of ana-budding speciation to       ################
##########            compare phyletic age and true age      ################
###############################################################################
...
###simulating species taxonomy; ana-budding speciation

Taxa.lam.01 <- lapply(Tree.lam.01, sim.taxonomy, beta = 0, lambda.a = 0.5*lam)
...
clado.df.01$mode <- rep(0, nrow(clado.df.01))
##anagenetic = 1; not anagenetic = 0
clado.df.01$anag <- rep(1, nrow(clado.df.01))
###extinction rates
clado.df.01$mu <- rep(mu.vector, each = Ntip)


##renaming columns
clado.df.ana.bud.01 <- clado.df.01 %>% rename(True.age = Age,
...
Taxa.lam.0.5 <- lapply(Tree.lam.05, sim.taxonomy, beta = 0, lambda.a = 0.5*lam)
...
##budding speciation = 0 ; bifurcating = 1
clado.df.0.5$mode <- rep(0, nrow(clado.df.0.5))
##anagenetic = 1; not anagenetic = 0
clado.df.0.5$anag <- rep(1, nrow(clado.df.0.5))
###extinction rates
clado.df.0.5$mu <- rep(mu.vector, each = Ntip)


##renaming columns
clado.df.ana.bud.0.5 <- clado.df.0.5 %>% rename(True.age = Age,
                                                Estimated.age = tip_length_reconstr,
                                                N_sisters = n_sisters_reconstr) 
...
Taxa.lam.1 <- lapply(Tree.lam.1, sim.taxonomy, beta = 0, lambda.a = 0.5*lam)
...
##budding speciation = 0 ; bifurcating = 1
clado.df.1$mode <- rep(0, nrow(clado.df.1))
##anagenetic = 1; not anagenetic = 0
clado.df.1$anag <- rep(1, nrow(clado.df.1))
###extinction rates
clado.df.1$mu <- rep(mu.vector, each = Ntip)


##renaming columns
clado.df.ana.bud.1 <- clado.df.1 %>% rename(True.age = Age,
                                            Estimated.age = tip_length_reconstr,
                                            N_sisters = n_sisters_reconstr) 
ana.bud.sim <- rbind(clado.df.ana.bud.01, clado.df.ana.bud.0.5, clado.df.ana.bud.1)

write_csv(ana.bud.sim, file = file.path(getwd(), pro, q_error, "ana.bud.sim.csv"))

bif.sim <- read_csv(file = file.path(getwd(), pro, q_error, "bif.sim.csv"))

###binding the dataframes
q.lambda <- rbind(bif.sim, ana.bif.sim, bud.sim, ana.bud.sim)

write_csv(q.lambda, file = file.path(getwd(), pro, q_error, "q.error.lambda.csv"))

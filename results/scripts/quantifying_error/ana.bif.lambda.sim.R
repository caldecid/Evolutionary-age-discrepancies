###############################################################################
###########    Simulation of ana-bifurcating speciation to  ################
...
###simulating species taxonomy; ana-bifurcating speciation

Taxa.lam.01 <- lapply(Tree.lam.01, sim.taxonomy, beta = 1, lambda.a = 0.5*lam)
...
##renaming columns
clado.df.ana.bif.01 <- clado.df.01 %>% rename(True.age = Age,
...
Taxa.lam.0.5 <- lapply(Tree.lam.05, sim.taxonomy, beta = 1, lambda.a = 0.5*lam)
...
##renaming columns
clado.df.ana.bif.0.5 <- clado.df.0.5 %>% rename(True.age = Age,
                                            Estimated.age = tip_length_reconstr,
                                            N_sisters = n_sisters_reconstr) 
...
Taxa.lam.1 <- lapply(Tree.lam.1, sim.taxonomy, beta = 1, lambda.a = 0.5*lam)
...
##renaming columns
clado.df.ana.bif.1 <- clado.df.1 %>% rename(True.age = Age,
                                        Estimated.age = tip_length_reconstr,
                                        N_sisters = n_sisters_reconstr) 
ana.bif.sim <- rbind(clado.df.ana.bif.01, clado.df.ana.bif.0.5, clado.df.ana.bif.1)

write_csv(ana.bif.sim, file = file.path(getwd(), pro, q_error, "ana.bif.sim.csv"))

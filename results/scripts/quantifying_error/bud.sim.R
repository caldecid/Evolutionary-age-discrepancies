###############################################################################
###########    Simulation of budding speciation to compare ################
...
Taxa.lam.01 <- lapply(Tree.lam.01, sim.taxonomy, beta = 0, lambda.a = 0)
...
##renaming columns
clado.df.bud.01 <- clado.df.01 %>% rename(True.age = Age,
...
Taxa.lam.0.5 <- lapply(Tree.lam.05, sim.taxonomy, beta = 0, lambda.a = 0)
...
##renaming columns
clado.df.bud.0.5 <- clado.df.0.5 %>% rename(True.age = Age,
                                            Estimated.age = tip_length_reconstr,
                                            N_sisters = n_sisters_reconstr) 
...
Taxa.lam.1 <- lapply(Tree.lam.1, sim.taxonomy, beta = 0, lambda.a = 0)
...
##renaming columns
clado.df.bud.1 <- clado.df.1 %>% rename(True.age = Age,
                                        Estimated.age = tip_length_reconstr,
                                        N_sisters = n_sisters_reconstr) 
bud.sim <- rbind(clado.df.bud.01, clado.df.bud.0.5, clado.df.bud.1)

write_csv(bud.sim, file = file.path(getwd(), pro, q_error, "bud.sim.csv"))
